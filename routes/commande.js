...
const stockage = multer.diskStorage({
  ...
  filename: function (request, fichier, callBack) {
    let uimage = uid.v4();
    return callBack(null, `${uimage + "_command_" + (Date.now()) + fichier.originalname}`)
  }
})
...
const upload = multer(
  {
    ...
    fileFilter: function (request, fichier, callBack) {
      ...
      if (extValide !== '.jpg' && extValide !== '.jpeg' && extValide !== '.png' && extValide !== '.tif' && extValide !== '.cr2' && extValide !== '.jfif' &&
        extValide !== '.JPG' && extValide !== '.JPEG' && extValide !== '.PNG' && extValide !== '.TIF' && extValide !== '.CR2' && extValide !== '.JFIF' && extValide !== '.pdf' && extValide !== '.PDF') {
